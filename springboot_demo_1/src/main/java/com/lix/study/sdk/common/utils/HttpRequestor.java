...
public class HttpRequestor {
	 ...
	 * @param config
	 */
	public HttpRequestor(Map<String, Integer> config) {
		if (config == null) {
			httpClient = HttpClients.createDefault();
		} else {
	...
	public String post(String url, String body, String contentType) {
		...
		post.setEntity(new StringEntity(body, DemoConts.UTF_8));
		return doExecute(post);
	}

	/**
	 * post请求 获取内容 请求参数以键值对方式提供
	 * @param url
	 * @return
	 */
	...
	private String doExecute(HttpUriRequest request) {
		try (CloseableHttpResponse response = (CloseableHttpResponse) httpClient.execute(request)) {
			...
			String result = EntityUtils.toString(entity, DemoConts.UTF_8);
	...
	private RequestConfig getRequestConfig() {
		return RequestConfig.custom().setConnectionRequestTimeout(MAX_READ_TIMEOUT).setConnectTimeout(MAX_CONN_TIMEOUT)
	...
	private HttpEntity getParamsEntity(Map<String, Object> params) {
		...
		try {
			return new UrlEncodedFormEntity(nvps, DemoConts.UTF_8);
	...
	private SocketConfig getSocketConfig(Map<String, Integer> config) {
		...
		if (config.containsKey(SO_KEEP_ALIVE)) {
			socBuilder.setSoKeepAlive(config.get(SO_KEEP_ALIVE).equals(1) ? true : false);
		}
		if (config.containsKey(TCP_NO_DELAY)) {
			socBuilder.setTcpNoDelay(config.get(TCP_NO_DELAY).equals(1) ? true : false);
		}
